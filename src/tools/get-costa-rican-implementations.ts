/**
 * get_costa_rican_implementations — Find Costa Rican statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetCostaRicanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface CostaRicanImplementationResult {
  document_id: string;
...
export async function getCostaRicanImplementations(
  db: InstanceType<typeof Database>,
  input: GetCostaRicanImplementationsInput,
): Promise<ToolResponse<CostaRicanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as CostaRicanImplementationResult[];
