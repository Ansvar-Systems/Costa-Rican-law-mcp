/**
 * search_eu_implementations — Search EU directives/regulations referenced by Costa Rican legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_costa_rican_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  costa_rican_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as costa_rican_statute_count
  ...
  if (input.has_costa_rican_implementation) {
    sql += ' HAVING costa_rican_statute_count > 0';
  }
