#!/usr/bin/env tsx
/**
 * Costa Rican Law MCP -- Ingestion Pipeline
 *
 * Fetches Costa Rican legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Costa Rican Parliament) provides free public access to all legislation
 ...
 * License: Costa Rican legislation is public domain under Art. 4 of the Copyright Act
 */
...
import { parseCostaRicanHtml, KEY_COSTA_RICAN_ACTS, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Costa Rican Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseCostaRicanHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Costa Rican Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  console.log('Costa Rican Law MCP -- Ingestion Pipeline');
  console.log('====================================\n');
  ...
  const acts = limit ? KEY_COSTA_RICAN_ACTS.slice(0, limit) : KEY_COSTA_RICAN_ACTS;
