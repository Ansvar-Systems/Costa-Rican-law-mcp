#!/usr/bin/env tsx
/**
 * Costa Rican Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.pgrweb.go.cr/scij';
const PORTAL_NAME = 'SCIJ (Sistema Costarricense de Informacion Juridica)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/costa-rican-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Costa Rican Law MCP — Data Freshness Check');
