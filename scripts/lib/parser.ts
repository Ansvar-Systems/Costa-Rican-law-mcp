/**
 * HTML parser for Costa Rican legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Costa Rican legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Costa Rican statute.
 *
 ...
 *   </div>
 */
export function parseCostaRicanHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Costa Rican acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Costa Rican legal text.
 *
 * Costa Rican definitions typically use patterns like:
